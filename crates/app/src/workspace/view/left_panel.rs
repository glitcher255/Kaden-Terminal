...
// Glitcher - Set availability to "Dummy", this_appst makes the panel contents empty but icons still exist
impl ToolPanelView {
...
impl View for LeftPanelView {
    fn ui_name() -> &'static str {"LeftPanelView"}

    // Render side panel UI buttons
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Hide toolbar buttons if there is only one as it's a bit redundant
        // Glitcher - Render left panel buttons
        let toolbelt_button_row = if self.toolbelt_buttons.len() > 1 {
            Some(
                Flex::row()
                    .with_cross_axis_alignment(CrossAxisAlignment::Center)
                    .with_spacing(4.0)
                    .with_children( self.toolbelt_buttons.iter().zip(&mouse_state_handles) // Glitcher, filter out warpdrive and conv buttons from toolbelt_buttons array
                     .filter(|(button_config, _)| {
            // Filter out by tooltip text or icon name
            !button_config.tooltip_text.contains("Warp Drive") 
                && !button_config.tooltip_text.contains("Agent conversations")
                     })
                    .map(
                        |(button_config, mouse_state)| {
                            Self::render_button(button_config, mouse_state.clone(), appearance)  // Render
                        },
                    ))
        ...
        let content_area: Box<dyn Element> = if availability != ToolPanelAvailability::Available {
            Shrinkable::new(1.0,self.render_unavailable_panel(appearance, active_view, availability),).finish()
        } 
        // Possibly handle panel content?
        else { 
            match active_view {
                ...
                ToolPanelView::GlobalSearch { .. } => match self.active_global_search_view(app) {
                    Some(global_search_view) => Shrinkable::new(1.0,Container::new(ChildView::new(&global_search_view).finish()).finish(),).finish(),
...
// Renderer - toolbar buttons and other, Not called directly
impl LeftPanelView {
    // Left panel button renderer
    fn render_button(button_config: &ToolbeltButtonConfig,mouse_state: MouseStateHandle,appearance: &Appearance,) -> Box<dyn Element> {
...
// Renderer, mostly for the content of panels?
impl LeftPanelView {
    // Render the content of panels with certain availability flags
    fn render_unavailable_panel(&self,appearance: &Appearance,view: ToolPanelView,availability: ToolPanelAvailability,) -> Box<dyn Element> {
        let (title, description) = match (view, availability) {
            ...
            _ => ("", ""),  // Glitcher - catch all for dummy returns - just returns empty pages instead of deleting them, still figuring that out
        };
    ...
    // Creates configuration for toolbar buttons
    fn create_toolbelt_button_config(view: &ToolPanelView,ctx: &ViewContext<Self>,) -> ToolbeltButtonConfig {
        match view {
            ToolPanelView::ProjectExplorer => {
    ...
    // Needs documentation
    pub fn new(working_directories_model: ModelHandle<WorkingDirectoriesModel>,views: Vec<ToolPanelView>,ctx: &mut ViewContext<Self>,) -> Self {
        ...
        let toolbelt_buttons = views
            .iter()
            // .filter(|view| {  // filter toolbelt_buttons to just have 2 elements for rendering later - does not do anything
            // !matches!(
            // view,
        ...
        ctx.subscribe_to_model(&working_directories_model, |me, _, event, ctx| {
            ...
            } = event
            {
                ...
                let file_tree_view = me.get_or_create_file_tree_view_for_pane_group(active_pane_group.id(), ctx);
                let is_visible = active_pane_group.as_ref(ctx).left_panel_open && me.is_file_tree_active();
    ...
    pub fn set_panel_position(&mut self,position: super::PanelPosition,ctx: &mut ViewContext<Self>,) {
    ...
    pub fn update_available_views(&mut self,views: Vec<ToolPanelView>,ctx: &mut ViewContext<Self>,) {
    ...
    fn get_or_create_global_search_view_for_pane_group(&mut self,pane_group_id: warpui::EntityId,ctx: &mut ViewContext<Self>,) -> ViewHandle<GlobalSearchView> {
    ...
    fn get_or_create_file_tree_view_for_pane_group(&mut self,pane_group_id: warpui::EntityId,ctx: &mut ViewContext<Self>,) -> ViewHandle<FileTreeView> {
        if let Some(view) = self
            .working_directories_model
            .as_ref(ctx)
    ...
    pub fn active_global_search_view(&self,app: &AppContext,) -> Option<ViewHandle<GlobalSearchView>> {
    ...
    pub(crate) fn auto_expand_active_file_tree_to_most_recent_directory(&mut self,ctx: &mut ViewContext<Self>,) {
    ...
    pub fn restore_active_view_from_snapshot(&mut self,view: ToolPanelView,ctx: &mut ViewContext<Self>,) {
        active_view_state::set(self, view, ctx);
    }

    /// Updates the active pane group ID so we filter events correctly.
    pub fn set_active_pane_group(&mut self,pane_group: ViewHandle<PaneGroup>,working_directories_model: &ModelHandle<WorkingDirectoriesModel>,ctx: &mut ViewContext<Self>,) {
    ...
    pub fn update_coding_panel_enablement(&mut self,enablement: CodingPanelEnablementState,ctx: &mut ViewContext<Self>,) {
        #[cfg(feature = "local_fs")]
        {
            if let Some(file_tree_view) = self.active_file_tree_view(ctx) {
                file_tree_view.update(ctx, |view, ctx| {
    ...
    fn handle_global_search_event(&mut self,_event: &GlobalSearchViewEvent,_ctx: &mut ViewContext<Self>,) {}

    #[cfg(feature = "local_fs")]
    fn handle_global_search_event(&mut self,event: &GlobalSearchViewEvent,ctx: &mut ViewContext<Self>,) {
        match event {
            GlobalSearchViewEvent::OpenMatch {
                ...
                /* send_telemetry_from_ctx!(TelemetryEvent::CodePanelsFileOpened {
...
// Mostly telemetry garbage
impl LeftPanelView {
    pub fn handle_action_with_force_open(&mut self,action: &LeftPanelAction,force_open: bool,ctx: &mut ViewContext<Self>,) {
        match action {
            LeftPanelAction::ProjectExplorer => {
                active_view_state::set(self, ToolPanelView::ProjectExplorer, ctx);
                if force_open {
                    /* send_telemetry_from_ctx!(TelemetryEvent::FileTreeToggled {
            ...
            LeftPanelAction::WarpDrive => {
                ...
                if self.active_view_availability(ctx) == ToolPanelAvailability::Available {
                    if force_open {
                        /* send_telemetry_from_ctx!(TelemetryEvent::WarpDriveOpened {
