...
use crate::{
    GlobalResourceHandlesProvider, TelemetryEvent, persistence, send_telemetry_sync_from_ctx,
};
...
impl AuthManager {
                ...
                ctx.spawn(
                    ...
                    |_, platform, _ctx| {
                        if let Some(_platform) = platform {
                            /* send_telemetry_from_ctx!(TelemetryEvent::DetectedIsolationPlatform { platform },
                            ctx); */
                        }
                    },
                );
    ...
    pub fn attempt_login_gated_feature(
        ...
        if self.auth_state.is_anonymous_or_logged_out() {
            /* send_telemetry_from_ctx!(TelemetryEvent::AnonymousUserAttemptLoginGatedFeature { feature },
            ctx); */
