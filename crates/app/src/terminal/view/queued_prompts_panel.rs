...
impl QueuedPromptsPanelView {
    ...
    pub(crate) fn commit_edit(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(_origin) = origin
            && !was_empty
        {
            /* send_telemetry_from_ctx!(TelemetryEvent::QueuedPromptEdited {
                origin: origin.into(),
            },
            ctx); */
        }
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            QueuedPromptsPanelAction::ToggleCollapsed => {
                ...
                /* send_telemetry_from_ctx!(TelemetryEvent::QueuedPromptPanelCollapseToggled {
            ...
            QueuedPromptsPanelAction::DeleteRow(query_id) => {
                ...
                if let Some(_removed) = removed {
                    /* send_telemetry_from_ctx!(TelemetryEvent::QueuedPromptDeleted {
            ...
            QueuedPromptsPanelAction::DropEnd => {
                ...
                if let (Some(from_index), Some(to_index), Some(_origin)) =
                    ...
                    && from_index != to_index
                {
                    /* send_telemetry_from_ctx!(TelemetryEvent::QueuedPromptReordered {
                        origin: origin.into(),
                        from_index,
                        to_index,
                    },
