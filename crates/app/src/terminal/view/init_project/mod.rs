    ...
    fn spawn_server_installation(
        ...
        ctx.spawn(
            ...
            move |_me, result, ctx| match result {
                Ok(()) => {
                    /* send_telemetry_from_ctx!(LspTelemetryEvent::ServerInstallCompleted {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            InitProjectBlockAction::IndexCodebase(directory) => {
                /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeSetupCodebaseContextAction {
            ...
            InitProjectBlockAction::SetupLanguageServers {
                ...
                for _server_type in enabled_servers.iter().chain(servers_to_install.iter()) {
                    /* send_telemetry_from_ctx!(LspTelemetryEvent::ServerEnabled {
                        server_type: server_type.binary_name().to_string(),
            ...
            InitProjectBlockAction::LinkFromExisting(path) => {
                let _file_name = path
                    ...
                    .unwrap_or("")
                    .to_string();
                /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeSetupProjectScopedRulesAction {
            ...
            InitProjectBlockAction::StartCreateEnvironment => {
                /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeSetupCreateEnvironmentAction {
