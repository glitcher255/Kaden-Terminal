...
use crate::server::telemetry::{
    ...
    QueuedPromptSendNowTrigger, TelemetryEvent,
};
...
impl Input {
    ...
    fn send_queued_row_immediately(
        ...
        if let Some(_origin) = origin {
            /* send_telemetry_from_ctx!(TelemetryEvent::QueuedPromptSentNow {
                origin: origin.into(),
                trigger,
            },
            ctx); */
    ...
    fn set_ai_context_menu_open(&mut self, open: bool, ctx: &mut ViewContext<Self>) {
        if FeatureFlag::AIContextMenuEnabled.is_enabled() && open {
            ...
            /* send_telemetry_from_ctx!(TelemetryEvent::AtMenuInteracted {
    ...
    fn toggle_legacy_slash_commands_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if is_slash_menu_open {
            ...
            let _is_in_agent_view = FeatureFlag::AgentView.is_enabled()
                && self.agent_view_controller.as_ref(ctx).is_fullscreen();
            /* send_telemetry_from_ctx!(TelemetryEvent::OpenSlashMenu {
    ...
    fn handle_conversation_menu_event(
        ...
        match event {
            InlineConversationMenuEvent::NavigateToConversation { item_id } => {
                let _is_in_agent_view = FeatureFlag::AgentView.is_enabled()
                    && self.agent_view_controller.as_ref(ctx).is_fullscreen();
                /* send_telemetry_from_ctx!(TelemetryEvent::InlineConversationMenuItemSelected { is_in_agent_view },
                ctx); */
    ...
    fn handle_inline_skill_selector_event(
        ...
        if self.skill_selector_should_invoke {
            ...
            /* send_telemetry_from_ctx!(SkillTelemetryEvent::Opened {
    ...
    fn open_conversation_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let _is_in_agent_view = FeatureFlag::AgentView.is_enabled()
            && self.agent_view_controller.as_ref(ctx).is_fullscreen();
        /* send_telemetry_from_ctx!(TelemetryEvent::InlineConversationMenuOpened { is_in_agent_view },
        ctx); */
        ctx.notify();
    }
    ...
    fn handle_user_query_menu_event(
        ...
        match event {
            ...
            UserQueryMenuEvent::AcceptedQuery {
                ...
                /* send_telemetry_from_ctx!(TelemetryEvent::SlashCommandAccepted {
    ...
    pub fn insert_zero_state_prompt_suggestion(
        ...
        /* send_telemetry_from_ctx!(TelemetryEvent::ZeroStatePromptSuggestionUsed {
            suggestion_type,
            triggered_from
        },
        ctx); */
        ...
        match event {
            ...
            UniversalDeveloperInputButtonBarEvent::InputTypeSelected(input_type) => {
                ...
                if switch_to_auto {
                    ...
                    /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeClickedEntrypoint {
    ...
    fn try_execute_command_from_source(
            ...
            .has_received_precmd()
        {
                ...
                }) = zerostate_next_command_suggestion_info
            {
                ...
                /* send_telemetry_from_ctx!(TelemetryEvent::AgentModePrediction {
                    was_suggestion_accepted: self.was_intelligent_autosuggestion_accepted,
                    request_duration_ms,
                    is_from_ai,
    ...
    fn handle_workflows_event(
        ...
        match event {
            ...
            workflows::CategoriesViewEvent::WorkflowSelected {
                ...
                // unused due to telemetry prune
                // let space: Option<cloud_objects::Space> = workflow_id.and_then(|id| {
                //     CloudViewModel::as_ref(ctx)
                //         .object_space(&id.to_string(), ctx)
                //         .map(Into::into)
                // });

                /* send_telemetry_from_ctx!(TelemetryEvent::WorkflowSelected(WorkflowTelemetryMetadata {
                    workflow_source,
        ...
        match event {
            WorkflowsInfoBoxViewEvent::PrefixCommandWithEnvironmentVariables(env_vars) => {
                ...
                if let Some(env_vars_id) = env_vars {
                    ...
                    /* send_telemetry_from_ctx!(TelemetryEvent::EnvVarWorkflowParameterization(telemetry_metadata),
                    ctx); */
                }
            }
        }
    ...
    fn handle_suggestions_event(
        ...
        match event {
            InputSuggestionsEvent::ConfirmSuggestion {
                ...
                /* send_telemetry_from_ctx!(TelemetryEvent::ConfirmSuggestion {
                    mode: self
                        .suggestions_mode_model
                        .as_ref(ctx)
                        .mode()
    ...
    fn maybe_send_autodetection_telemetry_on_manual_toggle(
        ...
        /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeChangedInputType {
            input,
            buffer_length,
        ...
        if matches!(new_input_type, InputType::Shell) && !ai_input_model.is_input_type_locked() {
            ...
            if !current_input_text.is_empty() {
                ...
                /* send_telemetry_from_ctx!(TelemetryEvent::AgentModePotentialAutoDetectionFalsePositive(event_payload),
                ctx); */
            }
        }
    }
    ...
    fn submit_ai_query_local(
        ...
        {
            /* send_telemetry_from_ctx!(TelemetryEvent::ExecutedWarpDrivePrompt {
    ...
    fn emit_input_buffer_submitted_telemetry(&self, ctx: &mut ViewContext<Self>) {
        ...
        /* send_telemetry_from_ctx!(TelemetryEvent::InputBufferSubmitted {
        ...
        let (workflow_id, workflow_command) = {
            match self.workflows_state.selected_workflow_state.as_ref() {
                Some(selected_workflow_state) => {
                    /* send_telemetry_from_ctx!(TelemetryEvent::WorkflowExecuted(WorkflowTelemetryMetadata {
    ...
    fn handle_action(&mut self, action: &InputAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            InputAction::ToggleInputAutoDetection => {
                ...
                {
                    /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeToggleAutoDetectionSetting {
