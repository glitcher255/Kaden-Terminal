...
// Glitcher, changed default keybinds to ctrl+c, ctrl+v, ctrl
//ctrl-# is wrong, need cmdorctrl-#
// Defaults terminalAction over-rides CustomAction here, be careful
...
pub fn custom_tag_to_keystroke(custom: CustomTag) -> Option<Keystroke> {
    match custom.into() {
        ...
        CustomAction::Copy => Keystroke::parse("cmdorctrl-shift-C").ok(),
        //  normal copy - 
        // You have to set it to ctrl-c then clear it for kill/copy to work
        //  using smart - C - init.rs -> app/src/terminal/view in function .with_key_binding("ctrl-c")
        // ctrl-# is wrong, need cmdorctrl-# - 
        // - defaults to CopyBlock ??
        // todo v1.1 - add both kill or copy to a customAction or Editablebinding
        // todo v1.1 - add kill only command to customAction or Editablebinding
        // todo v1.1 - Figure out why hotkeys are configured / hard coded across different files and not consolidated
        CustomAction::CopyBlock => Keystroke::parse("cmdorctrl-1").ok(),
        CustomAction::Paste => Keystroke::parse("ctrl-v").ok(),
        #[cfg(windows)]
        CustomAction::WindowsPaste => Keystroke::parse("ctrl-v").ok(),
        #[cfg(windows)]
        CustomAction::WindowsCopy => Keystroke::parse("cmdorctrl-c").ok(),
        CustomAction::Undo => Keystroke::parse("cmdorctrl-z").ok(),
        CustomAction::Redo => Keystroke::parse("cmdorctrl-shift-Z").ok(),
        CustomAction::ClearEditor => Keystroke::parse("ctrl-o").ok(),
