...
impl CodeReviewView {
    ...
    fn apply_diff_mode(&mut self, mode: DiffMode, ctx: &mut ViewContext<Self>) {
        ...
        /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::BaseChanged {
    ...
    fn show_find_bar(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::FindBarToggled {
            is_local: self.repo_is_local(),
            is_open: true,
        },
        ctx); */
    ...
    fn invalidate_all(
        ...
        /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::DiffLoadCompleted {
            is_local,
            mode: diff_mode,
    ...
    fn update_review_comment(
        ...
        if is_existing {
            /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::CommentEdited {
                is_local: self.repo_is_local(),
            },
            ctx); */
        } else {
            /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::CommentAdded {
                is_local: self.repo_is_local(),
            },
            ctx); */
        }
    ...
    fn delete_comment_by_id(&mut self, id: CommentId, ctx: &mut ViewContext<Self>) {
        if let Some(model) = self.active_comment_model.clone() {
            ...
            /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::CommentDeleted {
                is_local: self.repo_is_local(),
                is_imported,
            },
            ctx); */
    ...
    fn handle_local_code_editor_events(
        ...
        match event {
            LocalCodeEditorEvent::FileSaved { .. } => {
                /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::FileSaved {
                    is_local: self.repo_is_local(),
                },
                ctx); */
                ctx.notify();
    ...
    fn reposition_comments_in_file(&mut self, diff_mode: &DiffMode, ctx: &mut ViewContext<Self>) {
        ...
        if fallback_count > 0 {
            /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::CommentRelocationFailed {
                is_local: self.repo_is_local(),
                fallback_count,
            },
            ctx); */
        ...
        if !newly_imported_ids.is_empty() {
            ...
            /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::CommentsAttached {
                is_local: self.repo_is_local(),
                active_count,
                outdated_count,
            },
    ...
    pub fn handle_review_submission_result(
        ...
        match result {
            ReviewSubmissionResult::Success {
                ...
                /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::ReviewSubmitted {
                    is_local: self.repo_is_local(),
                    comment_count,
                    file_count,
                    destination,
    ...
    fn handle_code_editor_event(
        ...
        match event {
            ...
            CodeEditorEvent::DiffReverted => {
                /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::RevertHunkClicked {
                    is_local: self.repo_is_local(),
                },
                ctx); */
            ...
            CodeEditorEvent::CommentEditorOpened => {
                /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::CommentEditorOpened {
                    is_local: self.repo_is_local(),
                },
                ctx); */
            }
    ...
    fn insert_selection_as_context(
        ...
        if let Some(terminal_view) = self.attach_target_terminal(ctx) {
            ...
            if let Some(routing) = terminal_view.update(ctx, |tv, ctx| {
                ...
                /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::AddToContext {
                    is_local: self.repo_is_local(),
    ...
    fn handle_action(&mut self, action: &CodeReviewAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            CodeReviewAction::ToggleMaximize => {
                ...
                /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::PaneStateChanged {
                    is_local: self.repo_is_local(),
                    state_change,
                },
                ctx); */
            ...
            CodeReviewAction::OpenCommitDialog => {
                /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::GitButtonTriggered {
                    is_local: self.repo_is_local(),
