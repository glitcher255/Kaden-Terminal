...
impl AIBlock {
    ...
    fn on_output_status_update(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        match status {
            ...
            AIBlockOutputStatus::Complete { output } => {
                ...
                /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeCreatedAIBlock {
                    client_exchange_id,
    ...
    fn handle_complete_output(&mut self, output: &AIAgentOutput, ctx: &mut ViewContext<Self>) {
        ...
        if !surfaced_citations.is_empty() {
            /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeSurfacedCitations {
    ...
    fn handle_ask_user_question_view_event(
        ...
        match event {
            ...
            AskUserQuestionViewEvent::SpeedbumpPermissionChanged(permission) => {
                ...
                /* send_telemetry_from_ctx!(TelemetryEvent::ChangedAgentModeAskUserQuestionPermission {
    ...
    pub fn dismiss_pending_suggested_prompt(
        ...
        /* send_telemetry_from_ctx!(TelemetryEvent::UnitTestSuggestionCancelled {
            identifiers,
            interaction_source,
        },
        ctx); */
    ...
    fn accept_unit_test_suggestion(
        ...
        /* send_telemetry_from_ctx!(TelemetryEvent::UnitTestSuggestionAccepted {
            identifiers,
...
impl AIBlock {
    ...
    fn handle_insert_code_review_comments(
        ...
        if !self.model.is_restored() {
            /* send_telemetry_from_ctx!(CodeReviewTelemetryEvent::CommentsReceived {
        ...
        match action {
            ...
            AIBlockAction::OpenCitation(citation) => {
                ...
                if let Some(_citation) = citation.for_telemetry(ctx) {
                    /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeOpenedCitation {
                        citation,
                        block_id: self.client_ids.client_exchange_id.to_string(),
                        conversation_id: self.client_ids.conversation_id,
                        server_output_id,
                    ...
                    *checked = !*checked;
                    BlocklistAIPermissions::handle(ctx).update(ctx, |model, ctx| {
                        match model.set_should_autoexecute_readonly_commands(*checked, ctx) {
                            Ok(_) => {
                                /* send_telemetry_from_ctx!(TelemetryEvent::ToggledAgentModeAutoexecuteReadonlyCommandsSetting {
                                    src: AutonomySettingToggleSource::Speedbump,
                                    enabled: *checked,
                                },
                                ctx); */
                    ...
                    *checked = !*checked;
                    ...
                    BlocklistAIPermissions::handle(ctx).update(ctx, |model, ctx| {
                        match model.set_coding_permissions(permission, ctx) {
                            Ok(_) => {
                                /* send_telemetry_from_ctx!(TelemetryEvent::ChangedAgentModeCodingPermissions {
                                    src: AutonomySettingToggleSource::Speedbump,
                                    new: permission,
                                },
                                ctx); */
            ...
            AIBlockAction::Rated { is_positive } => {
                ...
                /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeRatedResponse {
