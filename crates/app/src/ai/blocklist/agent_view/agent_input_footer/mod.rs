        ...
        Fut: std::future::Future<Output = Result<(), PluginInstallError>> + Send + 'static,
    {
        ...
        ctx.spawn(
            ...
            move |me, result, ctx| {
                ...
                if result.is_ok() {
                    /* send_telemetry_from_ctx!(TelemetryEvent::CLIAgentPluginOperationSucceeded {
                    ...
                    /* send_telemetry_from_ctx!(TelemetryEvent::CLIAgentPluginOperationFailed {
                        cli_agent: agent.into(),
                        operation: operation_kind,
                    },
                    ctx); */
            ...
            VoiceInputLifecycleState::Idle => {
                ...
                match session_result {
                    Ok(session) => {
                        ...
                        if let Some(agent) = self.cli_agent(ctx) {
                            /* send_telemetry_from_ctx!(TelemetryEvent::CLIAgentToolbarVoiceInputUsed {
                                cli_agent: agent.into(),
                            },
                            ctx); */
                        }
        ...
        match action {
            ...
            AgentInputFooterAction::InsertFilePath(path) => {
                if let Some(_agent) = self.cli_agent(ctx) {
                    /* send_telemetry_from_ctx!(TelemetryEvent::CLIAgentToolbarImageAttached {
                        cli_agent: agent.into(),
                    },
                    ctx); */
                }
            ...
            AgentInputFooterAction::InstallPlugin => {
                ...
                {
                    if let Some(_agent) = self.cli_agent(ctx) {
                        /* send_telemetry_from_ctx!(TelemetryEvent::CLIAgentPluginChipClicked {
                            cli_agent: agent.into(),
            ...
            AgentInputFooterAction::DismissPluginChip => {
                ...
                {
                    /* send_telemetry_from_ctx!(TelemetryEvent::CLIAgentPluginChipDismissed {
                        cli_agent: agent.into(),
                        chip_kind: kind.into(),
                    },
                    ctx); */
