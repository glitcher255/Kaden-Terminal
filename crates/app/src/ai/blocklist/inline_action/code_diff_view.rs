...
impl CodeDiffView {
    ...
    fn setup_diff_view_subscriptions(
        ...
        ctx.subscribe_to_view(diff_view, move |me, _, event, ctx| match event {
            ...
            InlineDiffViewEvent::UserEdited => {
                ...
                /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeCodeSuggestionEditedByUser { output_id },
                ctx); */
            }
        });
    }
    ...
    fn try_accept_action_with_selection(
        ...
        match selection {
            AcceptSelection::Only => {
                /* send_telemetry_from_ctx!(RequestFileEditsTelemetryEvent::EditAcceptClicked(EditAcceptClickedEvent {
            ...
            AcceptSelection::AndContinueWithAgent => {
                /* send_telemetry_from_ctx!(RequestFileEditsTelemetryEvent::EditAcceptAndContinueClicked(
    ...
    fn navigate_to_diff_hunk(&mut self, direction: Direction, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(_output_id) = self.server_output_id() {
            /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeCodeDiffHunksNavigated { output_id },
            ctx); */
        }
    }
    ...
    fn select_file(&mut self, direction: Direction, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(_output_id) = self.server_output_id() {
            /* send_telemetry_from_ctx!(TelemetryEvent::AgentModeCodeFilesNavigated {
                output_id,
    ...
    fn send_telemetry_for_edit_resolution(
        ...
        /* send_telemetry_from_ctx!(RequestFileEditsTelemetryEvent::EditResolved(EditResolvedEvent {
            identifiers: self.identifiers.clone(),
            response,
            stats: EditStats {
    ...
    pub fn send_malformed_line_telemetry(&self, ctx: &mut ViewContext<Self>) {
        ...
        if correction_count > 0 {
            /* send_telemetry_from_ctx!(RequestFileEditsTelemetryEvent::MalformedFinalLineProxy(
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            CodeDiffViewAction::ScrollToExpand => {
                ...
                /* send_telemetry_from_ctx!(TelemetryEvent::ExpandedCodeSuggestions {
                    identifiers: self.identifiers.clone(),
                },
                ctx); */
            }
