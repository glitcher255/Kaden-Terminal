    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            CloudSetupGuideAction::CopyCode { code, step: _ } => {
                /* send_telemetry_from_ctx!(AgentManagementTelemetryEvent::SetupGuideStepCopy { step: *step },
                ctx); */
                ctx.clipboard()
                    .write(ClipboardContent::plain_text(code.clone()));
            }
            CloudSetupGuideAction::RunWorkflow { workflow, step: _ } => {
                /* send_telemetry_from_ctx!(AgentManagementTelemetryEvent::SetupGuideStepRun { step: *step },
                ctx); */
            ...
            CloudSetupGuideAction::VisitOz => {
                ctx.open_url(OZ_URL);
                /* send_telemetry_from_ctx!(AgentManagementTelemetryEvent::SetupGuideStepRun {
            ...
            CloudSetupGuideAction::OpenDocs { docs } => {
                ...
                ctx.open_url(url);
                /* send_telemetry_from_ctx!(AgentManagementTelemetryEvent::SetupGuideDocsLink { docs: *docs },
                ctx); */
            }
        }
    }
