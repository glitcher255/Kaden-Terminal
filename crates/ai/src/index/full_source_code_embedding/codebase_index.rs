        ...
        let abort_handle = ctx
            .spawn(
                ...
                move |me, incremental_update_sync_result, ctx| {
                    /* send_telemetry_from_ctx!(incremental_update_sync_result.telemetry_event(sync_start_time),
                    ctx); */
            ...
            .spawn(
                ...
                move |me, file_tree_result, ctx| {
                    match file_tree_result {
                        ...
                        Err(e) => {
                            report_error!(&e);
                            /* send_telemetry_from_ctx!(AITelemetryEvent::BuildTreeFailed {
                                error: e.to_string(),
                            },
                            ctx); */
        ...
        let abort_handle = ctx
            .spawn(
                ...
                move |me, (tree, server_sync_result), ctx| {
                    /* send_telemetry_from_ctx!(server_sync_result.telemetry_event(
                        sync_start_time.elapsed(),
        ...
        match index_build_result {
            Ok(IndexBuildResult {
                ...
                if let Some(sync_time) = time_tracker.compute_duration_for_interval(SYNC_TIME) {
                    /* send_telemetry_from_ctx!(server_sync_result
                ...
                {
                    /* send_telemetry_from_ctx!(AITelemetryEvent::BuildTreeSuccess {
            ...
            .spawn(
                ...
                move |me, load_result, ctx| match load_result {
                    Ok(SnapshotLoaded {
                        ...
                        let tree = *boxed_tree;
                        /* send_telemetry_from_ctx!(AITelemetryEvent::MerkleTreeSnapshotDiffSuccess {
                    ...
                    Err(SnapshotLoadError::DiffFailed(err)) => {
                        /* send_telemetry_from_ctx!(AITelemetryEvent::MerkleTreeSnapshotDiffFailed {
                            error: err.to_string()
                        },
                        ctx); */
                        report_error!(&err);
